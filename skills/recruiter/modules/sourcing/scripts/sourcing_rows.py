...
ALIASES = {
    ...
    "eligibility": ("eligibility", "eligible", "eligibility status"),
    "evidence_status": ("evidence_status", "evidence status"),
...
def generic_row(row: dict[str, Any]) -> list[str]:
    values = [
        ...
        get_value(row, "eligibility"),
        get_value(row, "evidence_status"),
...
def loxo_row(row: dict[str, Any], kind: str) -> list[str]:
    ...
    tags = "; ".join(part for part in ["Sourcing", kind.title(), eligibility, evidence_status, fit, supplied_tags] if part)
    notes = " | ".join(
        part
        for part in [
            get_value(row, "evidence"),
