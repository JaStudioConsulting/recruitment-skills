...
if ([capabilities, tools, plugins, hosts, workflows].every(Boolean)) {
  ...
  if (hosts.hosts?.some((host) => !["codex", "opencode", "claude", "hermes", "gemini"].includes(host.id) && host.active_repoint_allowed !== false)) problems.push("unapproved host cutover");
...
if (problems.length) { console.error([...new Set(problems)].sort().join("\n")); process.exitCode = 1; } else console.log(`validated standalone recruiter package: ${capabilities.capabilities.length} capabilities, ${tools.tools.length} declared MCP contract tools, ${hosts.hosts.length} hosts, ${files.length} skills files`);
