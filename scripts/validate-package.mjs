...
if ([capabilities, tools, plugins, hosts, workflows].every(Boolean)) {
  ...
  ids(workflows.workflows, "workflow", problems); for (const workflow of workflows.workflows || []) { if (workflow.router !== "recruiter") problems.push(`workflow bypasses recruiter: ${workflow.id}`); for (const id of [workflow.capability, ...(workflow.capabilities || [])].filter(Boolean)) if (!capabilityIds.has(id)) problems.push(`unknown workflow capability: ${id}`); for (const file of [workflow.fixture, workflow.template].filter(Boolean)) { const base = file.startsWith("recruiter/") ? skillsRoot : root; const target = path.resolve(base, file); if (!contained(base, target) || !await exists(target)) problems.push(`missing workflow file: ${file}`); } }
}
